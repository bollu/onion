...
bool has_token(const std::string &token_list, const std::string &token)
{
    ...
    {
        while (pos < token_list.size() && is_whitespace(token_list[pos]))
        {
            ++pos;
        }

        size_t end = pos;
        while (end < token_list.size() && !is_whitespace(token_list[end]))
        {
            ++end;
        }

        if (end > pos && token_list.compare(pos, end - pos, token) == 0)
        {
            return true;
        }
