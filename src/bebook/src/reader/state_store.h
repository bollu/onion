...
public:
    ...
    // Directory holding every persisted file, for subsystems (e.g. the library index)
    // that keep their own store alongside.
    const std::filesystem::path &get_base_dir() const;
