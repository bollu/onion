...
constexpr const char *SETTINGS_KEY_JUSTIFY = "justify";
constexpr const char *SETTINGS_KEY_HYPHENATE = "hyphenate";
...
std::optional<bool> settings_get_justify(const StateStore &state_store)
{
    const auto &opt = state_store.get_setting(SETTINGS_KEY_JUSTIFY);
    if (!opt)
    {
        return std::nullopt;
    }
...
void settings_set_justify(StateStore &state_store, bool justify)
{
    state_store.set_setting(SETTINGS_KEY_JUSTIFY, justify ? "true" : "false");
}

std::optional<bool> settings_get_hyphenate(const StateStore &state_store)
{
    const auto &opt = state_store.get_setting(SETTINGS_KEY_HYPHENATE);
    if (!opt)
    {
        return std::nullopt;
    }
...
void settings_set_hyphenate(StateStore &state_store, bool hyphenate)
{
    state_store.set_setting(SETTINGS_KEY_HYPHENATE, hyphenate ? "true" : "false");
}
