...
bool SettingsView::render(SDL_Surface *dest_surface, bool force_render)
{
    ...
    {
        text::Font *sys_font = cached_load_font(font_name, sys_styling.get_font_size());
        text::Font *user_font = sys_styling.get_loaded_font();
        ...
        auto render_text = [&](const char *str, int style, text::Font *font = nullptr) {
            return surface_unique_ptr { text::render_text_shaded(
                font ? font : sys_font,
                str,
                style == style_normal ?
                    theme.main_text :
