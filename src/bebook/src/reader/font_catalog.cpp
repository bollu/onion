...
void discover_fonts()
{
    ...
    // The italic and bold members of a family are not separately selectable fonts: the
    // text engine picks them up automatically as siblings of the regular face. Listing
    // them here would fill the font setting with entries that are really one family.
    auto is_style_variant = [](const std::string &stem) {
        static const char *const SUFFIXES[] = {
            "-Italic", "-Bold", "-BoldItalic", "-BoldIt", "-It",
            "-Oblique", "-Semibold", "-SemiBold", "-SemiBoldItalic",
            "-Medium", "-MediumItalic", nullptr
        };
    ...
    for (const auto &entry: directory_listing(FONT_DIR))
    {
        ...
        if (!entry.is_dir && test_font(path) && !is_style_variant(path.stem().string()))
        {
