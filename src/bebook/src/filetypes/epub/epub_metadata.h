...
// Bibliographic information from the OPF <metadata> element. Any field may be empty:
// epubs in the wild routinely omit the author, and only a minority declare a cover.
struct EpubMetadata
{
    std::string title;
    std::string author;
    std::string cover_href; // doc path in epub, empty if the book declares no cover
};
