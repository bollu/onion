...
bool EPubReader::open(DocReaderCache &cache)
{
    ...
    PackageContents &package = state->package;
    {
    ...
    if (navmap.empty())
    {
        auto nav_item = std::find_if(
            ...
            [](const auto &item) {
                return item.second.media_type == APPLICATION_XHTML_XML && has_token(item.second.properties, "nav");
            }
        );
...
const EpubMetadata &EPubReader::get_metadata() const
{
    return state->package.metadata;
}
...
{
    return read_zip_file(state->zip, path);
}
