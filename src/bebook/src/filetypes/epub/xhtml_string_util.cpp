...
std::string compact_strings(const std::vector<const char*> &strings)
{
    ...
    return compact_strings(strings, ignored);
}
...
)
{
    ...
    out_offsets.clear();
    out_offsets.reserve(strings.size());
    ...
    for (const char *str : strings)
    {
        ...
        out_offsets.push_back(static_cast<uint32_t>(result.size()));
    ...
    // The trailing strip can land behind offsets recorded for inputs that contributed
    // only whitespace at the very end.
    for (auto &offset : out_offsets)
    {
        offset = std::min<uint32_t>(offset, static_cast<uint32_t>(result.size()));
    }
