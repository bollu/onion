...
public:
    ...
    // Valid only once open() has succeeded. Deliberately not on DocReader: it is epub
    // specific, and plain text files have nothing to report.
    const EpubMetadata &get_metadata() const;
