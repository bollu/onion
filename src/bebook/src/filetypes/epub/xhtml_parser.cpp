...
const std::unordered_map<std::string, ElementType> _elem_name_to_enum {
    ...
    {"tr",    ElementType::Tr},
    {"td",    ElementType::Td},
    {"em",     ElementType::Emphasis},
    {"i",      ElementType::Emphasis},
    {"cite",   ElementType::Emphasis},
    {"dfn",    ElementType::Emphasis},
    {"var",    ElementType::Emphasis},
    {"strong", ElementType::Strong},
    {"b",      ElementType::Strong},
};
...
struct Node
{
    ...
    Node(Type type, DocAddr address, xmlNodePtr node, std::string text, int list_depth, text::Style style)
        : type(type)
        , address(address)
        , node(node)
        , text(std::move(text))
...
class NodeProcessor
{
    ...
    // Counted rather than flagged, since these elements nest and overlap freely.
    ...
    void emit_node(int node_depth, Node::Type type, xmlNodePtr node, std::string text = "")
    {
        ...
        nodes.emplace_back(type, current_address, node, std::move(text), list_depth, current_style());
            ...
            case Node::Type::InlineList:
                {
                    ...
                    // Whitespace compaction moves everything, so the styles have to be
                    // mapped onto the joined result by asking where each contributing
                    // text node ended up rather than by tracking lengths.
                    std::vector<uint32_t> node_offsets;
                    std::string text = compact_strings(substrings, node_offsets);
                    if (text.size())
                    {
                        ...
                        for (uint32_t j = 0; j < group_size; ++j)
                        {
                            ...
                            const uint32_t end = (j + 1 < group_size)
                                ? node_offsets[j + 1]
                                : static_cast<uint32_t>(text.size());
                            if (end > start)
                            {
                                style_runs.push_back(
                                    text::StyleRun{start, end - start, nodes[i + j].style}
                                );
                            }
                        }
                        text::normalize_runs(style_runs);

                        // Styling every character the same is the overwhelmingly common
                        // case; storing nothing lets the renderer skip the styled path.
